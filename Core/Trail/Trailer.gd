...
func create_trail():
	_trail = Trail.new()
	#_trail.texture = gradient_texture
	#_trail.emission_texture = gradient_texture
	_trail.min_segment_length = min_segment_length
	_trail.max_segment_length = max_segment_length
	_trail.two_sided = two_sided
	_trail.hot_time = lifetime
	_trail.color_gradient = gradient
	_trail.width_curve = width_curve
	_trail.default_width = width
	_trail.default_color = color
	_trail.material = material
	_trail.t_from_length = t_from_length
	_trail.keep_aspect = keep_aspect
	#_trail.line_shadow = cast_shadow
	_trail.cast_shadow = cast_shadow
